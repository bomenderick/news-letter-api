use news_letter_api::configuration::{get_configuration, DatabaseSettings};
...
// ensure that the 'tracing' stack is only initialize once using 'once_cell'
...
async fn spawn_app() -> TestApp {
    ...
    let server = run(tcp_listener, connection_pool.clone()).expect("Failed to bind to address.");
...
async fn subscribe_returns_a_200_for_valid_form_data() {
    ...
    assert_eq!(saved.email, "bomenderick@gmail.com");
    assert_eq!(saved.name, "bomen derick");
}
